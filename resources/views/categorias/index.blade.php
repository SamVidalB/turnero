...
@section('content')
    <div class="container">
        ...
        <div>
            <a href="{{ url('categorias/trash') }}" class="btn btn-danger float-end mt-2 ms-2">Papelera</a>
            <a href="{{ url('categorias/create') }}" class="btn btn-primary float-end mt-2">Crear Categoría</a>
            <h1>Categorías</h1>
        </div>
            ...
            <tbody>
                @foreach($data as $categoria)
                    <tr>
                        ...
                        <td>
                            <a href="{{ url('categorias/' . $categoria->id . '/edit') }}" class="btn btn-icon btn-outline-primary" title="Editar">
                                <span class="icon-base bx bx-edit icon-md"></span>
                            </a>
                            <form action="{{ url('categorias/' . $categoria->id) }}" method="POST" style="display:inline;">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="btn btn-icon btn-outline-primary" title="Eliminar" onclick="return confirm('¿Está seguro de que desea eliminar esta categoría?');">

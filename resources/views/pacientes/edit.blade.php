...
@section('content')
    <div class="container">
        ...
        <form id="formPaciente" action="{{ route('pacientes.update', $paciente->id) }}" method="post">
            @csrf
            @method('PUT')
            ...
            <div class="row">
                <div class="col-md-6 mb-3">
                    ...
                    <input type="text" class="form-control" id="nombre" name="nombre" required autofocus value="{{ old('nombre', $paciente->nombre) }}">
                    @error('nombre') <div class="error text-danger">{{ $message }}</div> @enderror
                </div>

                <div class="col-md-6 mb-3">
                    <label for="documento" class="form-label">Documento</label>
                    <input type="text" class="form-control" id="documento" name="documento" required value="{{ old('documento', $paciente->documento) }}">
            ...
            <div class="row">
                <div class="col-md-6 mb-3">
                    ...
                    <input type="date" class="form-control" id="fecha_nacimiento" name="fecha_nacimiento" required value="{{ old('fecha_nacimiento', $paciente->fecha_nacimiento) }}">
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <select class="form-select" id="asegurador_id" name="asegurador_id" required>
                        ...
                        @foreach($aseguradores as $asegurador)
                            <option value="{{ $asegurador->id }}" {{ old('asegurador_id', $paciente->asegurador_id) == $asegurador->id ? 'selected' : '' }}>
                                {{ $asegurador->nombre }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <select class="form-select" id="categoria_id" name="categoria_id" required>
                        ...
                        @foreach($categorias as $categoria)
                            <option value="{{ $categoria->id }}" {{ old('categoria_id', $paciente->categoria_id) == $categoria->id ? 'selected' : '' }}>
                                {{ $categoria->nombre }}
                            </option>
                        @endforeach
                    </select>

...
@section('content')
    <div class="container">
        ...
        <div>
            <a href="{{ url('usuarios/trash') }}" class="btn btn-danger float-end mt-2 ms-2">Papelera</a>
            <a href="{{ url('usuarios/create') }}" class="btn btn-primary float-end mt-2">Crear Usuario</a>
            <h1>Usuarios</h1>
        </div>
            ...
            <tbody>
                @foreach($data as $usuario)
                    <tr>
                        ...
                        <td>{{ $usuario->rol }}</td>
                        <td>
                            <a href="{{ url('usuarios/' . $usuario->id . '/edit') }}" class="btn btn-icon btn-outline-primary" title="Editar">
                                <span class="icon-base bx bx-edit icon-md"></span>
                            </a>
                            <form action="{{ url('usuarios/' . $usuario->id) }}" method="POST" style="display:inline;">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="btn btn-icon btn-outline-primary" title="Eliminar" onclick="return confirm('¿Está seguro de que desea eliminar este usuario?');">
